...
class HangmanAPI(object):
    ...
    def guess(self, word):  # word input example: "_ p p _ e "
        ...
        state = torch.tensor([word], dtype=torch.int64).to(self.device)     # shape = 1, MAX_WORD_LEN

        qvals = self.model(state).cpu().detach().numpy()

        qvals[0][
            [ACTION_MAPPING_STR_TO_INT[letter] for letter in self.guessed_letters]
        ] = float('-inf')                                                   # 不考虑history_action
        action = np.argmax(qvals)

        guess_letter = ACTION_MAPPING_INT_TO_STR[action]
        return guess_letter
...
if __name__ == "__main__":
    api = HangmanAPI(
        access_token="57146bb9cb29c8d6c26d07e0238ac3",
        timeout=2000,
